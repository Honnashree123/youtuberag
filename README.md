...
- **Transcript fetching fails in deployed apps**: YouTube blocks requests from cloud IPs (common on platforms like Streamlit Cloud). The code includes proxy fallback - configure your own proxy in the `get_transcript` function for deployment. The app works best when run locally on your machine.
